...
@section('description', 'Edit Vacancy Details')
@section('keywords', 'Edit, Vacancy')
            ...
            <div class="grid md:grid-cols-12 grid-cols-1 gap-[30px]">
                ...
                <div class="lg:col-span-8 md:col-span-7">
                    <div class="p-6 rounded-md shadow-lg bg-white dark:bg-gray-800 mt-8">
                        <h5 class="text-2xl font-semibold mb-6">Vakansiya məlumatlarını dəyişdir</h5>
                        <div class="notes-section text-black dark:text-gray-300 p-4 rounded-md border border-gray-300 mb-6">
                            <h6 class="text-lg font-semibold text-black dark:text-white mb-3">Qeyd</h6>
                            <p class="text-black dark:text-white">{{ $vacancy->notes }}</p>
                        </div>
                        <!-- Update Form -->
                        <form class="space-y-6" action="{{ route('vacancy_update', $vacancy->id) }}" method="post" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')  <!-- Add PUT method to indicate it's an update request -->
                            ...
                            <div class="grid lg:grid-cols-12 lg:gap-6">
                                ...
                                <div class="lg:col-span-6">
                                    ...
                                    <div class="relative mt-2">
                                        <input name="vacancy_name" id="vacancy_name" type="text"
                                               ...
                                               value="{{ old('vacancy_name', $vacancy->vacancy_name) }}">
                                    </div>
                                    @error('vacancy_name')
                                ...
                                <!-- Kateqoriya -->
                                <div class="lg:col-span-6">
                                    <label for="category_id" class="block font-medium text-gray-700">{{$words['category']->translate(app()->getLocale())->title}}:</label>
                                    <div class="relative mt-2">
                                        <select name="category_id" id="category_id" class="form-select w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-300 rounded-lg shadow-sm">
                                            ...
                                            @foreach($categories as $category)
                                                <option value="{{ $category->id }}" {{ old('category_id', $vacancy->category_id) == $category->id ? 'selected' : '' }}>{{ $category->title }}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    @error('category_id')
                                ...
                                <div class="lg:col-span-12">
                                    ...
                                    <div class="relative mt-2">
                                        <textarea name="requirements" id="requirements" class="form-input w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-300 rounded-lg shadow-sm" style="min-height: 150px;" placeholder="{{$words['requirement']->translate(app()->getLocale())->title}}">{{ old('requirements', $vacancy->requirements) }}</textarea>
                                    </div>
                                    @error('requirements')
                                    <span class="text-red-600">{{ $message }}</span>
                                    @enderror
                                ...
                                <div class="lg:col-span-12">
                                    ...
                                    <div class="relative mt-2">
                                        <textarea name="working_conditions" id="working_conditions" class="form-input w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-300 rounded-lg shadow-sm" style="min-height: 150px;" placeholder="{{$words['work_condition']->translate(app()->getLocale())->title}}">{{ old('working_conditions', $vacancy->working_conditions) }}</textarea>
                                    </div>
                                ...
                                <div class="lg:col-span-6">
                                    ...
                                    <div class="relative mt-2">
                                        <input name="application_method" id="application_method" type="text"
                                               class="form-input w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-300 rounded-lg shadow-sm"
                                               value="{{ old('application_method', $vacancy->application_method) }}" placeholder="Email və ya telefon">
                                    </div>
                                ...
                                <div class="lg:col-span-6">
                                    ...
                                    <div class="relative mt-2">
                                        <select name="is_negotiable" id="is_negotiable_select" class="form-select w-full border-gray-300 focus:border-emerald-500 focus:ring focus:ring-emerald-300 rounded-lg shadow-sm">
                                            <option value="0" {{ old('is_negotiable', $vacancy->is_negotiable) == 0 ? 'selected' : '' }}>{{$words['no']->translate(app()->getLocale())->title}}</option>
                                            <option value="1" {{ old('is_negotiable', $vacancy->is_negotiable) == 1 ? 'selected' : '' }}>{{$words['yes']->translate(app()->getLocale())->title}}</option>
                                        </select>
                                    </div>
                                    @error('is_negotiable')
                                    <span class="text-red-600">{{ $message }}</span>
